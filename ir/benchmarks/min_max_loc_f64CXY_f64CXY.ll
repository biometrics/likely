; ModuleID = 'likely'

%u0CXYT = type { i32, i32, i32, i32, i32, i32, [0 x i8] }
%f64CXY = type { i32, i32, i32, i32, i32, i32, [0 x double] }

; Function Attrs: nounwind
declare noalias %u0CXYT* @likely_new(i32 zeroext, i32 zeroext, i32 zeroext, i32 zeroext, i32 zeroext, i8* noalias nocapture) #0

; Function Attrs: nounwind
declare void @llvm.assume(i1) #0

; Function Attrs: nounwind
define %f64CXY* @min_max_loc(%f64CXY*) #0 {
entry:
  %1 = getelementptr inbounds %f64CXY, %f64CXY* %0, i64 0, i32 2
  %channels = load i32, i32* %1, align 4, !range !0
  %2 = tail call %u0CXYT* @likely_new(i32 28992, i32 %channels, i32 3, i32 2, i32 1, i8* null)
  %3 = getelementptr inbounds %f64CXY, %f64CXY* %0, i64 0, i32 3
  %columns = load i32, i32* %3, align 4, !range !0
  %4 = getelementptr inbounds %f64CXY, %f64CXY* %0, i64 0, i32 4
  %rows = load i32, i32* %4, align 4, !range !0
  %5 = zext i32 %channels to i64
  %dst_y_step = mul nuw nsw i64 %5, 3
  %6 = getelementptr inbounds %u0CXYT, %u0CXYT* %2, i64 1
  %7 = bitcast %u0CXYT* %6 to double*
  %8 = ptrtoint %u0CXYT* %6 to i64
  %9 = and i64 %8, 31
  %10 = icmp eq i64 %9, 0
  tail call void @llvm.assume(i1 %10)
  %channels1 = load i32, i32* %1, align 4, !range !0
  %src_c = zext i32 %channels1 to i64
  %src_x = zext i32 %columns to i64
  %11 = getelementptr inbounds %f64CXY, %f64CXY* %0, i64 0, i32 6, i64 0
  %12 = ptrtoint double* %11 to i64
  %13 = and i64 %12, 31
  %14 = icmp eq i64 %13, 0
  tail call void @llvm.assume(i1 %14)
  %15 = shl nuw nsw i64 %5, 1
  %16 = shl nuw nsw i64 %5, 2
  %17 = mul nuw nsw i64 %5, 5
  br label %c_body

c_body:                                           ; preds = %end, %entry
  %c = phi i64 [ 0, %entry ], [ %c_increment, %end ]
  br label %then

then:                                             ; preds = %c_body, %end6
  %storemerge26 = phi i32 [ 0, %c_body ], [ %62, %end6 ]
  %18 = phi double [ 0x7FEFFFFFFFFFFFFF, %c_body ], [ %55, %end6 ]
  %19 = phi double [ 0xFFEFFFFFFFFFFFFF, %c_body ], [ %59, %end6 ]
  %20 = phi i32 [ 0, %c_body ], [ %54, %end6 ]
  %21 = phi i32 [ 0, %c_body ], [ %53, %end6 ]
  %22 = phi i32 [ 0, %c_body ], [ %58, %end6 ]
  %23 = phi i32 [ 0, %c_body ], [ %57, %end6 ]
  %24 = sext i32 %storemerge26 to i64
  %25 = mul nsw i64 %24, %src_x
  br label %then5

end:                                              ; preds = %end6
  %26 = getelementptr double, double* %7, i64 %c
  store double %55, double* %26, align 8, !llvm.mem.parallel_loop_access !1
  %27 = sitofp i32 %54 to double
  %28 = add nuw nsw i64 %c, %5
  %29 = getelementptr double, double* %7, i64 %28
  store double %27, double* %29, align 8, !llvm.mem.parallel_loop_access !1
  %30 = sitofp i32 %53 to double
  %31 = add nuw nsw i64 %c, %15
  %32 = getelementptr double, double* %7, i64 %31
  store double %30, double* %32, align 8, !llvm.mem.parallel_loop_access !1
  %33 = add nuw nsw i64 %c, %dst_y_step
  %34 = getelementptr double, double* %7, i64 %33
  store double %59, double* %34, align 8, !llvm.mem.parallel_loop_access !1
  %35 = sitofp i32 %58 to double
  %36 = add nuw nsw i64 %c, %16
  %37 = getelementptr double, double* %7, i64 %36
  store double %35, double* %37, align 8, !llvm.mem.parallel_loop_access !1
  %38 = sitofp i32 %57 to double
  %39 = add nuw nsw i64 %c, %17
  %40 = getelementptr double, double* %7, i64 %39
  store double %38, double* %40, align 8, !llvm.mem.parallel_loop_access !1
  %c_increment = add nuw nsw i64 %c, 1
  %c_postcondition = icmp eq i64 %c_increment, %5
  br i1 %c_postcondition, label %c_exit, label %c_body, !llvm.loop !1

c_exit:                                           ; preds = %end
  %41 = bitcast %u0CXYT* %2 to %f64CXY*
  ret %f64CXY* %41

then5:                                            ; preds = %then, %then5
  %storemerge125 = phi i32 [ 0, %then ], [ %60, %then5 ]
  %42 = phi double [ %18, %then ], [ %55, %then5 ]
  %43 = phi double [ %19, %then ], [ %59, %then5 ]
  %44 = phi i32 [ %20, %then ], [ %54, %then5 ]
  %45 = phi i32 [ %21, %then ], [ %53, %then5 ]
  %46 = phi i32 [ %22, %then ], [ %58, %then5 ]
  %47 = phi i32 [ %23, %then ], [ %57, %then5 ]
  %48 = sext i32 %storemerge125 to i64
  %tmp = add i64 %48, %25
  %tmp2 = mul i64 %tmp, %src_c
  %49 = add i64 %tmp2, %c
  %50 = getelementptr %f64CXY, %f64CXY* %0, i64 0, i32 6, i64 %49
  %51 = load double, double* %50, align 8, !llvm.mem.parallel_loop_access !1
  %52 = fcmp olt double %51, %42
  %53 = select i1 %52, i32 %storemerge26, i32 %45
  %54 = select i1 %52, i32 %storemerge125, i32 %44
  %55 = select i1 %52, double %51, double %42
  %56 = fcmp ogt double %51, %43
  %57 = select i1 %56, i32 %storemerge26, i32 %47
  %58 = select i1 %56, i32 %storemerge125, i32 %46
  %59 = select i1 %56, double %51, double %43
  %60 = add nuw nsw i32 %storemerge125, 1
  %61 = icmp eq i32 %60, %columns
  br i1 %61, label %end6, label %then5

end6:                                             ; preds = %then5
  %62 = add nuw nsw i32 %storemerge26, 1
  %63 = icmp eq i32 %62, %rows
  br i1 %63, label %end, label %then
}

attributes #0 = { nounwind }

!0 = !{i32 1, i32 -1}
!1 = distinct !{!1}
