...
Assignments are \dindex{mutable} using the \pindex{assign} statement.
...
\subsection{(<- \emph{name expression})}
The \dindex{assign} statement stores the value of \emph{expression} into the variable \emph{name} previously defined by an \pindex{allocate} expression.
...
\subsection{this}
The \dindex{this} operand refers to the current \dindex{environment} of referenceable variables.
...
min-element :=
  src :->
  {
    ...
    check-column :=
      (x y t) :->
        (iter (-> c (check-element c x y t)) src.channels)

    check-row :=
      (y t) :->
        (iter (-> x (check-column x y t)) src.columns)

    check-frame :=
      t :->
        (iter (-> y (check-row y t)) src.rows)

    (iter (-> t (check-frame t)) src.frames)
