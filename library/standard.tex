...
Compile-time function evaluation \index{compile-time function evaluation|see {CTFE}} (CTFE) \index{CTFE} is a core tenant of the Likely programming language.
...
One particularly salient use of the JIT compiler is for CTFE \index{CTFE}, as the JIT compiler automatically leverages all instruction set extensions and coprocessors available at runtime.
...
Though algorithms are often written without specifying parameter types, they are still statically compiled because CTFE \index{CTFE} output dictates input type, and often dimensionality as well.
...
The computer vision domain and Likely's library design are well suited for \emph{live coding} \index{live coding} popularized by \fref{http://worrydream.com/}{Bret Victor}.
...
Likely is a \fref{http://www.literateprogramming.com/}{literate programming} \index{literate programming} language.
...
Likely recognizes both \fref{https://help.github.com/articles/github-flavored-markdown}{GitHub Flavored Markdown} \index{GitHub Flavored Markdown|see {GFM}} (GFM) \index{GFM} and \fref{http://www.latex-project.org/}{\LaTeX} \index{latex} syntax, and will automatically extract source code from appropriately marked blocks.

\part{Language Reference}
\chapter{Parsing}
This chapter covers how source code files are translated into the compiler's \emph{abstract syntax tree} \index{abstract syntax tree|see {AST}} (AST) \index{AST} program representation.

\section{Source Code File Formats}
To facilitate literate programming,\index{literate programming} Likely recognizes several source code file formats based on their file extension:

\begin{figure}[h]
\centering
\begin{description}
\item[.lisp] - The entire file is Likely source code.
\item[.md]   - GFM \index{GFM} file, source code is extracted from \texttt{likely} syntax-highlighted code blocks.
\item[.tex]  - \LaTeX \index{latex} file, source code is extracted from \texttt{likely} environment.
\end{description}
\caption{Source code file formats.}
\end{figure}

The Likely API function call for inferring file format from extension is \texttt{likely\char`_guess\char`_file\char`_type} \index{\texttt{likely\char`_guess\char`_file\char`_type}}.

\section{Lexical Analysis}
Source code is first converted into a sequence of words or \emph{tokens} \index{token} by applying the following rules:

\begin{figure}[h]
\centering
\begin{enumerate}
\item Tokens are separated by whitespace.
\item The following special single-character tokens need not be separated by whitespace:\newline
      \texttt{();.:}
...
The Likely API function call for lexical analysis is \texttt{likely\char`_lex} \index{\texttt{likely\char`_lex}}.

\section{Prefix Notation}
Likely is a member of the \noun{Lisp} family of languages, accepting fully-parenthesized Polish \emph{prefix notation}:
...
An important property of prefix notation is that it is equivalent to AST, where branches are \emph{lists} and leaves are \emph{atoms}.
