...
\begin{likely}
new := (-> return-type
           (-> (channels columns rows frames data)
...
decode := (-> return-type
              (-> mat ((extern return-type "likely_decode" (u8CXYT type-t)) mat return-type)))
...
\begin{likely}
read-image               := (-> file-name ((read image)           file-name media    ))
read-image-grayscale     := (-> file-name ((read image-grayscale) file-name media    ))
read-video               := (-> file-name ((read video)           file-name media    ))
read-video-grayscale     := (-> file-name ((read video-grayscale) file-name media    ))
read-directory           := (-> file-name ((read video)           file-name directory))
read-directory-grayscale := (-> file-name ((read video-grayscale) file-name directory))
\end{likely}
...
import := (-> (file environment) ((read text) file guess).data.(import-string file.guess-file-type environment))
\end{likely}
