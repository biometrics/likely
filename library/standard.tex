...
Both the \fref{\doxygenurl console\_interpreter\_compiler.html}{\texttt{likely} console-interpreter-compiler} and the \fref{\doxygenurl integrated\_development\_environment.html}{\texttt{dream} integrated development environment} are built on top of the \fref{\doxygenurl index.html}{\noun{Likely C API}}.
...
\section{Portability}
Likely is written in a portable subset of \noun{C++}, which means it can \fref{http://llvm.org/docs/GettingStarted.html\#hardware}{run everywhere \LLVM\ does}.
...
The computer vision domain and Likely's library design are well suited for the idea of \emph{live coding} \index{live coding} popularized by \fref{http://worrydream.com/}{Bret Victor}.
...
Likely is a \cref{http://www.literateprogramming.com/}{\emph{literate programming}}{knuth1984literate} \index{literate programming} language.
...
Likely recognizes both \fref{https://help.github.com/articles/github-flavored-markdown}{GitHub Flavored Markdown} (\GFM) and \cref{http://www.latex-project.org/}{\LaTeX}{lamport1986document} \index{\LaTeX} syntax, and will automatically extract source code from appropriately marked blocks.

\part{Language Reference}
\chapter{Parsing}
This chapter covers how source code files are translated into the compiler's \emph{abstract syntax tree} (\AST) program representation.

\section{Source Code File Formats}
To facilitate \pindex{literate programming}, Likely recognizes several source code file formats based on their file extension:
...
\end{description}
\caption{Source code file extensions formats.}
\end{figure}
...
Similarly, this \LaTeX\ \index{\LaTeX}\fref{\githuburl library/literate\_programming.tex}{source file} can be rendered \fref{https://s3.amazonaws.com/liblikely/latex/literate\_programming.pdf}{to \noun{PDF}} and also executed by Likely:

\begin{lstlisting}[language=bash, caption={Literate programming using \LaTeX \index{\LaTeX}.}, captionpos=b]
$ likely library/literate_programming.tex
42
\end{lstlisting}

\pindex{\texttt{likely\char`_guess\char`_file\char`_type}} is the Likely API function call for inferring file format from extension.
...
The Likely API function call for lexical analysis is \pindex{\texttt{likely\char`_lex}}.

\section{Prefix Notation}
Likely is a member of the Lisp family of languages, accepting fully-parenthesized Polish \emph{prefix notation}:
...
An important property of prefix notation is that it is equivalent to the \AST, where branches are \emph{lists} and leaves are \emph{atoms}.
