...
A \pindex{kernel} operator may not be nested within another \pindex{kernel} or \pindex{reduction} operator.
...
A \pindex{reduction} operator may not be nested within another \pindex{reduction} or \pindex{kernel} operator.
...
iter :=
  (expr end) :->
    (iter-range expr (end.type 0) end)
...
iter-reverse :=
  (expr end) :->
    (iter-reverse-range expr end (end.type 0))
...
iter-cuboid :=
  (f x y z) :->
    (-> i (-> j (-> k (f k j i)).(iter x)).(iter y)).(iter z)
\end{likely}
...
sum :=
  src :->
  {
    sum := (src.element-type.depth-atleast-32 0).$
    check-element :=
      e :->
        sum :<- (+ (sum) e)
    src:iter-elements check-element
...
average-channel :=
  src :->
    (average-generic (src.type.not-indirect.depth-atleast-32.floating.not-multi-channel 1 src.columns src.rows src.frames) src src.channels)

average-column :=
  src :->
    (average-generic (src.type.not-indirect.depth-atleast-32.floating.not-multi-column src.channels 1 src.rows src.frames) src src.columns)

average-row :=
  src :->
    (average-generic (src.type.not-indirect.depth-atleast-32.floating.not-multi-row src.channels src.columns 1 src.frames) src src.rows)

average-frame :=
  src :->
    (average-generic (src.type.not-indirect.depth-atleast-32.floating.not-multi-frame src.channels src.columns src.rows 1) src src.frames)
\end{likely}
...
transpose :=
  src :->
  {
    dst := (src.type.not-indirect src.channels src.rows src.columns src.frames)
    (dst src) :=>
      (dst c x y) :<- (src c y x)
  }
...
mtimes :=
  (A B) :->
  {
    ...
    C := (A.type.not-indirect 1 B.columns A.rows)
    len := A.columns
    (C A B len) :=>
      C :<- (dot (-> i (A 0 i y))
                 (-> i (B 0 x i))
