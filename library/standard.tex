...
% The Likely tokenizer will look for the use of \begin{likely} and \end{likely} to
...
\emph{Likely} is a programming language for developing image processing and statistical learning algorithms.
...
Likely relies on the \href{http://llvm.org/}{Low Level Virtual Machine} (LLVM) just-in-time (JIT) compiler infrastructure for optimizations traditionally impossible in statically compiled languages.
Algorithms automatically leverage all instruction set extensions and coprocessors available at runtime.
Furthermore, the entire premise of Likely is hinged on the observation that for image recognition applications, \emph{while algorithms must be generic to handle any matrix data type, at runtime they tend to be repeatedly executed on the same type.}
...
Most importantly however, \emph{statistical learning moves from an offline model generation step to a compile-time simplification of a function (the learning algorithm) with constant arguments (the training set)}.

Compile-time knowledge of the statistical model opens the door to \href{https://github.com/biometrics/likely/issues/20}{unexplored optimization opportunities}.

\subsection{Portability}
Likely is written using a portable subset of \emph{ISO C++11}, which means it can \href{http://llvm.org/docs/GettingStarted.html#hardware}{run everywhere LLVM does}.
Algorithms execute natively on single CPU threads via the LLVM \emph{Machine Code Just-In-Time} (MCJIT) compiler, multi-core CPUs via a custom \emph{OpenMP}-like backend, and GPUs and coprocessors via \emph{CUDA} or \emph{OpenCL}.

\begin{tabular}{ l l }
Parallelization & Status \\ \hline
Single-thread   & Done   \\
Multi-thread    & Done   \\
CUDA            & \href{https://github.com/biometrics/likely/issues/24}{\#24} \\
OpenCL          & \href{https://github.com/biometrics/likely/issues/25}{\#25} \\
\end{tabular}

Likely algorithms can be shipped as minimalist native libraries, written for dynamic just-in-time applications and scripts, integrate into other languages, published as web services, and transcompiled into single \emph{C} or \emph{JavaScript} source files.

\begin{tabular}{ l l }
Deployment               & Status \\ \hline
Static compiler          & Done   \\
JIT compiler             & Done   \\
REPL shell               & Done   \\
SWIG                     & \href{https://github.com/biometrics/likely/issues/46}{\#46} \\
Web services             & \href{https://github.com/biometrics/likely/issues/44}{\#44} \\
C transcompiler          & \href{https://github.com/biometrics/likely/issues/51}{\#51} \\
JavaScript transcompiler & \href{https://github.com/biometrics/likely/issues/45}{\#45} \\
\end{tabular}
...
This problem domain and Likely's JIT approach seem well suited for the concept of emph{live coding} popularized by \href{http://worrydream.com/}{Bret Victor}.
In fact, Likely's accompanying IDE called \emph{dream}, is \href{https://www.youtube.com/watch?v=a_hz8wFACVM}{designed from the ground up to support interactive algorithm development}.

\subsection{Literate Programming}
Likely is a \href{http://www.literateprogramming.com/}{literate programming} language.
...
Likely uses \href{https://help.github.com/articles/github-flavored-markdown}{Github Flavored Markdown} (GFM) syntax, and the Likely runtime will automatically extract and execute code block found in GFM source files.

\section{Language Reference}
\subsection{Prefix Notation}
Likely is a Lisp-like language accepting fully-parenthesized Polish prefix notation, or \emph{s-expressions}:
...
\subsection{Abstract Syntax Tree (AST) Manipulation}
Likely recognizes three special tokens that influence how source code is parsed into an AST.
These \emph{optional} tokens allow a developer to selectively depart from fully-parenthesized Polish prefix notation in order to improve code readability.
These tokens and parenthesis are the \emph{only} symbols that need not be separated by a space.
...
\subsubsection{Intrinsic Operator}
...
\subsection{Intrinsic Operators}
...
\subsubsection{External Symbols}
...
\subsection{Basic mathematical constants}
\begin{likely}
null  := 0
true  := 1
false := 0
...
\subsection{Basic unary functions}
\begin{likely}
not  := (-> a (== a false))
bool := (-> a (!= a false))
sq  := (-> a (* a a))
...
\subsection{Basic binary functions}
\begin{likely}
