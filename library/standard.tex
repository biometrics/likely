...
imitate-channels   := (imitate-dimension multi-channel)
imitate-columns    := (imitate-dimension multi-column)
imitate-rows       := (imitate-dimension multi-row)
imitate-frames     := (imitate-dimension multi-frame)
imitate-dimensions := (imitate-dimension multi-dimension)
\end{likely}
...
iter-channels :=
  expr :->
    (-> (mat t y x) (iter (-> c (expr mat t y x c)) mat.channels))

iter-columns :=
  expr :->
    (-> (mat t y) (iter (-> x (expr mat t y x)) mat.columns))

iter-rows :=
  expr :->
    (-> (mat t) (iter (-> y (expr mat t y)) mat.rows))

iter-frames :=
  expr :->
    (-> mat (iter (-> t (expr mat t)) mat.frames))

iter-elements :=
  (mat expr) :->
    (expr.iter-channels.iter-columns.iter-rows.iter-frames mat)
\end{likely}
...
min-element :=
  src :->
  {
    ...
    check-element :=
      (mat t y x c) :->
        current-min-value :<- (min current-min-value (mat c x y t))
...
max-element :=
  src :->
  {
    ...
    check-element :=
      (mat t y x c) :->
        current-max-value :<- (max current-max-value (mat c x y t))
