...
\subsection{Subtraction (- $lhs$ $[rhs]$)}
...
\subsection{Remainder (\% $num$ $denom$)}
...
\subsection{Less-than (\textless\ $lhs$ $rhs$)}
...
\subsection{Less-than or Equal-to (\textless = $lhs$ $rhs$)}
...
\subsection{Greater-than (\textgreater\ $lhs$ $rhs$)}
...
\subsection{Greater-than or Equal-to (\textgreater = $lhs$ $rhs$)}
...
\subsection{Bitwise-or (\textbar\ $a$ $b$)}
...
\subsection{Bitwise-exclusive-or (\char`\^\ $a$ $b$)}
...
\subsection{Left-shift (\textless\textless\ $x$ $n$)}
...
\subsection{Right-shift (\textgreater\textgreater\ $x$ $n$)}
