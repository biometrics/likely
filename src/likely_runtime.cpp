...
likely_size likely_elements(likely_mat m)
{
    return m->channels * m->columns * m->rows * m->frames;
}

likely_size likely_bytes(likely_mat m)
{
...
likely_mut likely_new(likely_type type, likely_size channels, likely_size columns, likely_size rows, likely_size frames, likely_data data, int8_t copy)
{
    likely_mut m;
    ...
    if (recycledBuffer) {
        ...
        if (recycledDataBytes >= dataBytes) { m = (likely_mut) recycledBuffer; dataBytes = recycledDataBytes; }
        else                                  m = (likely_mut) realloc((void*) recycledBuffer, headerBytes + dataBytes);
        recycledBuffer = NULL;
    } else {
        m = (likely_mut) malloc(headerBytes + dataBytes);
    }
...
likely_mut likely_scalar(double value)
{
    likely_mut m = likely_new(likely_type_from_value(value), 1, 1, 1, 1, NULL, 0);
    likely_set_element(m, value, 0, 0, 0, 0);
    return m;
}

likely_mut likely_copy(likely_mat m)
{
    return likely_new(m->type, m->channels, m->columns, m->rows, m->frames, m->data, true);
}

likely_mut likely_retain(likely_mat m)
{
...
double likely_element(likely_mat m, likely_size c, likely_size x, likely_size y, likely_size t)
{
...
void likely_set_element(likely_mut m, double value, likely_size c, likely_size x, likely_size y, likely_size t)
{
    if (!m) return;
    const likely_size columnStep = m->channels;
    const likely_size rowStep = m->columns * columnStep;
...
static likely_mat thunkMatricies[LIKELY_NUM_ARITIES+1];
...
static void executeWorker(size_t id)
{
    ...
    typedef void (*likely_kernel_1)(const likely_mat, likely_mat, likely_size, likely_size);
    typedef void (*likely_kernel_2)(const likely_mat, const likely_mat, likely_mat, likely_size, likely_size);
    typedef void (*likely_kernel_3)(const likely_mat, const likely_mat, const likely_mat, likely_mat, likely_size, likely_size);
...
extern "C" LIKELY_EXPORT void likely_fork(void *thunk, likely_arity arity, likely_size size, likely_mat src, ...)
{
